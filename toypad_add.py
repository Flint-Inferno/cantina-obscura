...
GH_LIVE_FOLDER    = 'live-page-links'
...
def _gh_token():
    """Get the current gh CLI auth token, running as the real user (not root)."""
    sudo_user = os.environ.get('SUDO_USER', '')
    try:
        if sudo_user:
            cmd = ['sudo', '-u', sudo_user, 'env',
                   f'HOME=/home/{sudo_user}', 'gh', 'auth', 'token']
        else:
            cmd = ['gh', 'auth', 'token']
...
def _gh_api(method, path, data=None, token=None):
...
def create_github_live_page(name, token):
    """Create live-page-links/{name}.html on GitHub from the mission template.

    Fetches mission-template.html from the repo via the GitHub API and PUTs the
    new file into live-page-links/.  Returns the GitHub Pages URL on success,
    or None on failure.
    """
    import urllib.error

    filepath = f'{GH_LIVE_FOLDER}/{name}.html'

    # Fetch template (content is base64 from the API)
    print("  Fetching mission template from GitHub...")
    try:
        tmpl = _gh_api('GET', f'/repos/{GH_OWNER}/{GH_REPO}/contents/mission-template.html', token=token)
    ...
    sha = None
    try:
        existing = _gh_api('GET', f'/repos/{GH_OWNER}/{GH_REPO}/contents/{filepath}', token=token)
        sha = existing.get('sha')
        ans = input(f"  '{name}.html' already exists. Overwrite? (y/n): ").strip().lower()
        if ans != 'y':
            return f'{GITHUB_PAGES_BASE}/{filepath}'
    except urllib.error.HTTPError as e:
        if e.code != 404:
    ...
    try:
        _gh_api('PUT', f'/repos/{GH_OWNER}/{GH_REPO}/contents/{filepath}', data=body, token=token)
...
def _update_live_page_difficulty(uid, difficulty, token):
    """Fetch the live page for uid, update DIFFICULTY:, push back. Returns True on success."""
    import re, base64
    path = f'/repos/{GH_OWNER}/{GH_REPO}/contents/{GH_LIVE_FOLDER}/{uid}.html'
    try:
        data     = _gh_api('GET', path, token=token)
        html     = base64.b64decode(data['content'].replace('\n', '')).decode('utf-8', errors='replace')
        patched  = re.sub(r'DIFFICULTY:\s*[^<&|]+', f'DIFFICULTY: {difficulty} ', html)
        if patched == html:
            print(f'  (DIFFICULTY field not found in page — skipped update)')
            return False
        encoded  = base64.b64encode(patched.encode()).decode()
        _gh_api('PUT', path, token=token, data={
            'message': f'Set DIFFICULTY: {difficulty} on {uid}.html via bulk enroll',
            'content': encoded,
            'sha':     data['sha'],
        })
        return True
...
def ask_url(label="Paste URL", existing=None, uid=None):
    ...
    while True:
        ...
        if choice == 'S':
            ...
            token = _gh_token()
            if not token:
                print("  ERROR: Could not get GitHub token. Is gh auth set up?")
                continue
            url = create_github_live_page(name, token)
            if url:
                print(f"\n  Tandem page URL: {url}")
                return url
...
def bulk_enroll_with_difficulty(dev):
    print("\n── Bulk Register + Set Difficulty ──")

    difficulty = input("  Difficulty for this batch (e.g. EASY, MEDIUM, HARD, EXTREME): ").strip().upper()
    if not difficulty:
        print("  No difficulty entered — cancelled.")
        return

    token = _gh_token()
    if not token:
        print("  ERROR: Could not get GitHub token via gh CLI. Is gh auth set up?")
        return

    print(f"\n  Difficulty: {difficulty}")
    print(f"  Scans tags, registers URL in tags.json, and updates the live page on GitHub.")
    ...
    try:
        while True:
            ...
            if uid in tags_data['tags'] and tags_data['tags'][uid] == url:
                print(f"  Already registered — updating difficulty only.")
            else:
                tags_data['tags'][uid] = url
                save_tags(tags_data)
                enrolled += 1
                print(f"  Registered in tags.json.")

            print(f"  Updating live page difficulty to '{difficulty}'...", end=' ', flush=True)
            ok = _update_live_page_difficulty(uid, difficulty, token)
            if ok:
                updated += 1
                print("Done.")
            _scanner_beep()
            ...
            pad.flash_color(dev, pad_id, 0, 255, 0, count=2)
            print(f"  Scan next tag...")
            time.sleep(1)
            pad.set_color(dev, pad_id, 0, 0, 0)

    except KeyboardInterrupt:
        print(f"\n  Done. {enrolled} tag(s) registered, {updated} live page(s) updated.")
...
def main():
    ...
    while True:
        ...
        print("  (6) Bulk register tags + set difficulty on live pages")
        print("  (q) Quit")
        ...
        if choice == '1':
            ...
            bulk_enroll_with_difficulty(dev)
        elif choice == 'q':
            break
        else:
